...
def conf(db=None):
    ...
    new, better function. It also doesn't return the CNF data, since we rarely need it, and we can always get it from cache_cnf if we want it.'''
    cache_cnf(db=db)
...
default is to use cache_cnf('~/.my.cnf')
    ...
    try:
        ...
        # In fact, this causes a connection error on cs.whitman.edu
        # which is why it is commented out below.
